...
export default function BurgerDashboard({ burgers }) {
    // KPIs
    const avgRating = burgers.length > 0 ? (burgers.reduce((sum, b) => sum + (b.rating || 0), 0) / burgers.length).toFixed(1) : '0.0';
    const totalSpent = burgers.reduce((sum, b) => sum + (b.precio || 0), 0).toFixed(0);
    const topRated = burgers.length > 0 ? [...burgers].sort((a, b) => b.rating - a.rating)[0]?.nombre_lugar?.slice(0, 8) : '-';
    ...
    <div className="w-full max-w-md space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-lg">
          ...
          <p className="text-3xl font-black text-gray-900">${totalSpent}</p>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-lg">
          ...
          <p className="text-3xl font-black text-gray-900">{topRated}</p>
